...
def mock_paths():
    ...
    with patch("roboclaw.config.loader.get_config_path") as mock_cp, \
         patch("roboclaw.config.loader.save_config") as mock_sc, \
         patch("roboclaw.config.loader.load_config") as mock_lc, \
         patch("roboclaw.cli.commands.get_workspace_path") as mock_ws:
...
def mock_agent_runtime(tmp_path):
    ...
    with patch("roboclaw.config.loader.load_config", return_value=config) as mock_load_config, \
         patch("roboclaw.config.paths.get_cron_dir", return_value=cron_dir), \
         patch("roboclaw.cli.commands.sync_workspace_templates") as mock_sync_templates, \
         patch("roboclaw.cli.commands._make_provider", return_value=object()), \
...
def test_agent_config_sets_active_path(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "roboclaw.config.loader.set_config_path",
        lambda path: seen.__setitem__("config_path", path),
    )
    monkeypatch.setattr("roboclaw.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("roboclaw.config.paths.get_cron_dir", lambda: config_file.parent / "cron")
    ...
    monkeypatch.setattr("roboclaw.cli.commands._print_agent_response", lambda *_args, **_kwargs: None)
...
def test_gateway_uses_workspace_from_config_by_default(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("roboclaw.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr(
        "roboclaw.cli.commands.sync_workspace_templates",
        lambda path: seen.__setitem__("workspace", path),
    )
    monkeypatch.setattr(
        "roboclaw.cli.commands._make_provider",
...
def test_gateway_workspace_option_overrides_config(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("roboclaw.config.loader.set_config_path", lambda _path: None)
    monkeypatch.setattr("roboclaw.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr(
        "roboclaw.cli.commands.sync_workspace_templates",
        lambda path: seen.__setitem__("workspace", path),
...
def test_gateway_uses_config_directory_for_cron_store(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("roboclaw.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("roboclaw.bus.queue.MessageBus", lambda: object())
    monkeypatch.setattr("roboclaw.session.manager.SessionManager", lambda _workspace: object())
    ...
    monkeypatch.setattr("roboclaw.cron.service.CronService", _StopCron)
