...
class AgentLoop:
    ...
    async def _handle_restart(self, msg: InboundMessage) -> None:
        ...
        async def _do_restart():
            ...
            # Use -m roboclaw instead of sys.argv[0] for Windows compatibility
            # (sys.argv[0] may be just "roboclaw" without full path on Windows)
    ...
    async def _process_message(
        ...
        if msg.channel == "system":
            ...
            self._schedule_background(self.episode_memory.maybe_distill(self.provider, self.model))
        ...
        for m in messages[skip:]:
            ...
            if role == "tool":
                if isinstance(content, list):
                    # Multimodal tool result — strip images for session storage, keep text
                    text_parts = [b["text"] for b in content if b.get("type") == "text"]
                    entry["content"] = "\n".join(text_parts) or "[multimodal result]"
